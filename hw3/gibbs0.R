...
MH_graphical <- function(data, theta, s = 0.1, lam = 0.2, niter = 1000,thinning = 1){
	...
	likeli_log <- function(data, theta){
		...
		Zs = sum((data_z %*% Theta) * data_z) ## of size 2^d
		Z = sum(exp(Zs))
	...
	prior_log <- function(theta, lam){
	  print(sum(dlaplace(theta,0,lam, log = T)))
		return(sum(dlaplace(theta,0,lam, log = T)))
	}

	posterir_log <- function(data, theta){
	  browser()
		return(likeli_log(data, theta) + prior_log(theta, lam))
	}

	accept <- function(data, theta_, theta){
	  # print("pos")
	  # print(posterir_log(data, theta))
	  # print(posterir_log(data, theta_))
	  
		prob = exp(posterir_log(data, theta_) - posterir_log(data, theta))
		return(min(1, prob))
	}
	...
	for (i in 1:niter) {
  		...
  		for (j in 1:thinning) {
    		...
    		alpha= accept(data,theta_, theta)
    		print(alpha)
    		if(is.nan(alpha)){
    		  return(theta)
    		}
...
test = MH_graphical(samples, theta, s = 0.1, lam = 0.2, niter = 1000,thinning = 1)
